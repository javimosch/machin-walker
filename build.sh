...
MODS="src/pbd.src src/body.src src/walk_task.src ml/vendor/tinybrain.src game/walker_game.src"
if pkg-config --exists raylib 2>/dev/null || [ -f /usr/include/raylib.h ]; then
    "$MACHIN" encode $MODS > walker.mfl
else
    ...
    "$MACHIN" encode $MODS | sed "s#header \"raylib.h\"#cflags \"-I${INC} -L${LIB}\" header \"raylib.h\"#; s#link \"raylib\"#link \":libraylib.a\"#" > walker.mfl
fi
"$MACHIN" build walker.mfl -o walker-game
rm -f walker.mfl
echo "built ./walker-game — run from the repo root (loads ml/models/walker.json)"
